...
// `null` means the legend needs no addition. German wording is deliberate,
// not a slip of the "code stays English" rule: `adr/0004` §5.3's note on
// Otto's answer of 20.09.2026 says the German terms for `complete` /
// `truncated` / `sample` "bleiben die Sprache dieses ADR und der
// Oberfläche" — they are the UI's own vocabulary, translated once, here.
export function completenessNote(result: CoverageResponse): string | null {
  ...
  const table: Record<CoverageCompleteness, () => string | null> = {
    complete: () => null,
    truncated: () => (total === null ? 'gekappt' : `zeigt ${counted} von ${total} Aufnahmen`),
    sample: () => (total === null ? 'Stichprobe' : `Stichprobe: ${counted} von ${total} Aufnahmen`),
  };
