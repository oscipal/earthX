...
| `earthx:format` | Speicherformat aus der Registry (`DataFormat`: `zarr`, `cog`, `legacy`, 6.2) — keine Interpretation, nur die Abbildung des vorhandenen Feldes. Damit unterscheidet das Frontend eine ganze Szene, die es direkt von der Quelle verlinken darf (COG, eine Datei), von einem Speicher, der es nicht ist (Zarr), ohne eine datensatzspezifische Stelle (M3-17) |
