...
SENTINEL_2 = ViewerInfo(group_by=("datetime", "grid:code"))
...
class TestAnInstantBecomesItsUtcDate:
    def test_the_time_of_day_drops_out_of_the_key(self) -> None:
        ...
        assert group_key(item(datetime="2026-07-24T10:38:17.453000Z"), ViewerInfo(group_by=("datetime",))) == (
            "2026-07-24",
        )
    ...
    def test_a_second_across_midnight_is_another_day(self) -> None:
        ...
        key = ViewerInfo(group_by=("datetime",))
    ...
    def test_an_instant_that_is_already_a_datetime_is_read_the_same_way(self) -> None:
        ...
        assert group_key(parsed, ViewerInfo(group_by=("datetime",))) == ("2026-07-24",)


class TestEverythingElseIsItsOwnText:
    def test_a_grid_code_is_carried_over_unchanged(self) -> None:
        key = group_key(item(**{"grid:code": "MGRS-32TMS"}), ViewerInfo(group_by=("grid:code",)))
        assert key == ("MGRS-32TMS",)

    def test_a_bare_date_stays_the_date_it_is(self) -> None:
        assert group_key(item(acquired="2026-07-24"), ViewerInfo(group_by=("acquired",))) == ("2026-07-24",)

    def test_a_number_becomes_its_text_rather_than_a_type_error(self) -> None:
        assert group_key(item(orbit=137), ViewerInfo(group_by=("orbit",))) == ("137",)
    ...
    def test_the_parts_keep_the_order_the_registry_names(self) -> None:
        ...
        assert group_key(scene, ViewerInfo(group_by=("grid:code", "datetime"))) == (
            "MGRS-32TMS",
            "2026-07-24",
        )
